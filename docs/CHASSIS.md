...
`pre_checkout`: рубильник, тень, сверка payload и каталога. Обычный бан → `ok=True`.  
`successful_payment` всегда пишет чек (деньги уже сняты), в том числе при рубильнике и бане.
...
6. Тень на admin/superadmin запрещена. Последнего суперадмина нельзя забанить и снять роль.  
